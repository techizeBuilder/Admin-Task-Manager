...
const RecurringTaskManager = () => {
  ...
  const handleDelete = (id) => {
    const task = currentTasks.find(t => t.id === id || t._id === id);
    setShowDeleteConfirmation({ id, task });
  };

  const confirmDelete = async () => {
    if (!showDeleteConfirmation) return;

    const { id } = showDeleteConfirmation;
    try {
      const task = currentTasks.find(t => t.id === id || t._id === id);
      const taskId = task?._id || id;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center flex-wrap gap-1 mb-3">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${getStatusPill(
                        task.isActive
                      )}`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-gray-600 truncate">
                        {task.nextDue ? new Date(task.nextDue).toLocaleDateString() : '—'}
                      </span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <Clock className="h-3 w-3 text-gray-400" />
                      <span className="text-gray-600 truncate">{task.estimatedTime || '—'}</span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <Tag className="h-3 w-3 text-gray-400" />
                      <span className="text-gray-600 truncate">{task.createdBy || '—'}</span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <Clock className="h-3 w-3 text-gray-400" />
                      <span className="text-gray-600 truncate">{task.lastGenerated ? new Date(task.lastGenerated).toLocaleDateString() : '—'}</span>
                    </div>
                  </div>
      ...
      <AlertDialog open={!!showDeleteConfirmation} onOpenChange={(open) => !open && setShowDeleteConfirmation(null)}>
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>"{showDeleteConfirmation?.task?.title}"</strong>? This will permanently remove the recurring task template and stop all future task generation. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
